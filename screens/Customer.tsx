...
export type CustomerSreenNavigationProp = StackNavigationProp<
  MainStackParamList,
  "Customer"
>;

const Customer = () => {
  const navigation = useNavigation<CustomerSreenNavigationProp>();
  ...
  const handleGenerateSeeding = async () => {
    try {
      const res = await CustomerService.generateSeeding();
      const { EC } = res;
      if (EC === 0) {
        const newCount = loopedCount + 1;
        setLoopedCount(newCount);

        // Only show modal when we've completed all generations
  ...
  return (
    <View>
      <FFScreenTopSection title="Customer" navigation={navigation} />
