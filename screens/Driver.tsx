...
export type DriverScreenNavigationProp = StackNavigationProp<
  MainStackParamList,
  "Driver"
>;
...
const Driver = () => {
  ...
  const handleGenerateSeeding = async () => {
    try {
      const res = await DriverService.generateSeeding();
      const { EC } = res;
      if (EC === 0) {
  ...
  return (
    <View>
      <FFScreenTopSection title="Driver" navigation={navigation} />
